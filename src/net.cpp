...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Digigems " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
      {"54.69.225.67", "54.69.225.67"},
    {NULL, NULL}
};

static const char *strTestNetDNSSeed[][2] = {
      {"88.196.13.22", "88.196.13.22"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Digigems is probably already running."), addrBind.ToString().c_str());
        else
