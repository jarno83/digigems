...
#if defined(NDEBUG)
# error "Digigems cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5d86c544206be84964dcd3cf26db3a1812477d0b6dd32469b51a61d8750ada42");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Digigems: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Digigems Signed Message:\n";
...
{
    // Digigems: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Digigems: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Digigems: 5 mins
static const int64 nTargetSpacing = 1 * 30; // Digigems: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Digigems: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Digigems: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5d86c544206be84964dcd3cf26db3a1812477d0b6dd32469b51a61d8750ada42");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "23. december. Its snowing in Estonia.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040184710aa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1419361241;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1104115;
        ...
        assert(block.hashMerkleRoot == uint256("0x648b7ea6fe00fec4b20e22659bd5f01869eb138706f2dc5a191d8f1e164b7ef7"));
...
unsigned char pchMessageStart[4] = { 0xff, 0xc2, 0xb8, 0xde }; // Digigems: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DigigemsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DigigemsMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DigigemsMiner(CWallet *pwallet)
{
    printf("DigigemsMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("digigems-miner");
    ...
    try { loop {
        ...
        printf("Running DigigemsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DigigemsMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DigigemsMiner, pwallet));
}
